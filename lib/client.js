...
Client.prototype.connect = function(cb) {
  ...
  if (this._autoDetect) {
    ...
    this._conns = [];
    this.emit('log', 'info', 'Connecting to cassandra hosts');
    this._seeds.forEach(function(host) {
      ...
      for (var i = 0; i < self._connsPerHost; i++) {
...
Client.prototype.connectPeers = function(seedConn, cb) {
  var self = this;
  this.emit('log', 'debug', 'Getting peers to connect');
  seedConn.query('SELECT peer FROM system.peers', function(err, result) {
    if (err) {
      self.emit('error', err);
      if (cb) {
        cb(err);
    ...
    this.emit('log', 'debug', 'Connecting to peers', { peers: rows });
    var onConnect = function() {
      if (cb) {
        cb();
        cb = null;
...
Client.prototype.close = function() {
  this.emit('log', 'info', 'Closing connections');
...
Client.prototype.createConnection = function(host, autoReconnect) {
  ...
  // bind events
  conn.on('close', this.emit.bind(this, 'log', 'info', 'Connection closed from cassandra', { host: host }));
  conn.on('connect', this.emit.bind(this, 'log', 'debug', 'Connected to cassandra', { host: host }));
  conn.on('reconnecting', this.emit.bind(this, 'log', 'debug', 'Reconnecting to cassandra', { host: host }));
  conn.on('error', function(err) {
    if (err && err.code === 'ECONNREFUSED') {
      // just log when connection refused
      self.emit('log', 'warn', 'Failed to connect to cassandra', { host: host });
    } else {
      self.emit('error', err);
    }
  });
...
Client.prototype.execute = function(query, values, options, callback) {
  ...
  this.emit('log', 'debug', 'Execute query', { query: query, values: values, options: options });
...
Client.prototype.batch = function() {
  ...
  return {
    ...
    commit: function(callback) {
      var _self = this;
      self.emit('log', 'debug', 'Commit batch', { commits: list });
      self.getAvailableConnection(function(err, conn) {
        if (err) {
          return callback(err);
        }
        // prepare with 4 concurrent
        async.mapLimit(list, 4, function(item, done) {
          conn.prepare(item.query, done);
        }, function(err, prepareds) {
          if (err && err._closed) {
            // retry query with closed error
            return setImmediate(_self.commit.bind(_self, callback));
          } else if (err) {
            return callback(err);
          }
          ...
          conn.batch(commits, opts, function(err) {
            if (err && err._closed) {
              ...
              setImmediate(_self.commit.bind(_self, callback));
