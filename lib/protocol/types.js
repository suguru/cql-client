...
ListType.prototype.deserialize = function(bytes) {
  ...
  if (bytes) {
    ...
    for (i = 0; i < num; i++) {
      itemLen = bytes.readUInt16BE(pos);
      pos += 2;
      item = bytes.slice(pos, pos + itemLen);
      pos += itemLen;
...
MapType.prototype.deserialize = function(bytes) {
  ...
  if (bytes) {
    ...
    num = bytes.readUInt16BE(0);
    for (i = 0; i < num; i++) {
      klen = bytes.readUInt16BE(pos);
      pos += 2;
      k = bytes.slice(pos, pos + klen);
      pos += klen;
      vlen = bytes.readUInt16BE(pos);
      pos += 2;
      v = bytes.slice(pos, pos + vlen);
      pos += vlen;
...
var types = {};
types.custom = new BytesType();
types.ascii = new TextType('ascii');
types.bigint = new LongType();
types.blob = types.custom;
types.boolean = new BooleanType();
types.counter = types.bigint;
types.decimal = new DecimalType();
types.double = new DoubleType();
types.float = new FloatType();
types.int = new Int32Type();
types.text = new TextType('utf8');
types.varchar = types.text;
types.varint = new IntegerType();
types.timestamp = new TimestampType();
types.uuid = new UUIDType();
types.timeuuid = types.uuid;
types.inet = new InetAddressType();
types.list = ListType.create;
types.map = MapType.create;
types.set = types.list;
